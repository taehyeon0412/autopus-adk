...
- 출력은 JSON (`YieldOutput` 구조)으로 아래 필드 포함:
  - `session_id`: 후속 collect/cleanup에 사용할 세션 ID
  - `panes`: `{ "provider_name": "pane_id" }` 맵
  - `round_history[0].responses[]`: 프로바이더별 Round 1 결과
- 프로바이더 실패 시 graceful degradation — 나머지 프로바이더 결과로 계속 진행
- Bash 호출이 에러를 반환한 경우에만 사용자에게 fallback 여부를 확인

> **⏭ POST-STEP**: JSON 파싱 후 `session_id`와 각 프로바이더의 `output`을 추출하여 Step 3.5로 진행.

### Step 3.5: Review Round 1 Results with Project Context

주 세션이 Round 1 결과를 프로젝트 컨텍스트와 함께 리뷰합니다:

1. Step 3 JSON에서 각 프로바이더의 SCAMPER 아이디어와 HMW 질문을 추출
2. `ARCHITECTURE.md`, 관련 SPEC, 기존 코드 패턴을 참조하여 실현 가능성을 평가
3. 각 프로바이더에게 보낼 **enriched rebuttal prompt**를 준비:
   - 다른 프로바이더의 아이디어 중 보완/발전시킬 포인트
   - 프로젝트의 기존 아키텍처/의존성에 비추어 구체화할 방향
   - 누락된 관점(보안, 성능, UX 등) 보강 요청

### [REQUIRED] Step 3.6: Send Rebuttal to Each Pane (MUST call Bash tool)

각 프로바이더에게 rebuttal 프롬프트를 전송합니다:

```bash
auto orchestra inject --session-id {session_id} --provider {provider_name} "{rebuttal prompt}"
```

- Step 3.5에서 준비한 프로바이더별 rebuttal을 각각 전송
- `inject` 커맨드가 없는 경우 fallback: `cmux send --surface {pane_id} "{prompt}"` 직접 사용
- 모든 프로바이더에게 전송 후 응답 대기

### Step 3.7: Collect Round 2 Results

Round 2 응답을 수집합니다:

```bash
auto orchestra collect --session-id {session_id} --round 2
```

- JSON 형태로 Round 2 프로바이더별 응답을 수신
- 수집 실패 시 Round 1 결과만으로 Step 4 진행 (degradation)

### Step 3.8: Cleanup Panes

세션의 모든 pane을 정리합니다:

```bash
auto orchestra cleanup --session-id {session_id}
```

- 이 단계는 Step 4 **이전에** 실행하여 리소스를 해제
- cleanup 실패는 경고만 출력하고 계속 진행

### Step 4: Main-Session Judge — ICE Scoring and Top N Selection

IMPORTANT: 주 세션이 직접 judge 역할을 수행합니다. Round 1 + Round 2 결과를 종합하여 프로젝트 컨텍스트 기반으로 아이디어를 통합하고 ICE 점수로 수렴합니다.

#### 4-1. 프로바이더 응답 통합

Round 1과 Round 2의 모든 프로바이더 아이디어를 병합합니다. 중복은 통합하되 유사한 아이디어도 보존합니다. 발산적 아이디어는 부록에 포함합니다 (AUGMENT, not FILTER).

#### 4-2. 프로젝트 컨텍스트 기반 ICE 스코어링

`ARCHITECTURE.md`, 기존 의존성, 코드 패턴을 참조하여 실현 가능성을 반영한 ICE 점수:

| 항목 | 설명 | 범위 |
|------|------|------|
| Impact | 영향력 | 1-10 |
| Confidence | 확신도 (프로젝트 적합성 포함) | 1-10 |
| Ease | 실행 용이성 (기존 코드 재활용 가능성 포함) | 1-10 |

`Score = (Impact × Confidence × Ease) / 100`

상위 N개 아이디어를 선별하여 BS 파일에 기록합니다.

#### 4-3. Assumption Risk Overlay
