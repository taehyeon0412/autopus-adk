...
동일한 16개 에이전트. 동일한 37개 스킬. 동일한 규칙. **어디서나.**
...
<details>
<summary><strong>CLI 명령어</strong> (루트 28개, 서브커맨드 포함 55개+)</summary>
...
| `auto test` | E2E 시나리오 실행기 (생성, 실행, 검증) |
| `auto react` | Reaction Engine (CI 실패 감지, 수정 보고서) |
| `auto agent create` | Meta-agent 빌더 (커스텀 에이전트 스캐폴드) |
| `auto skill create` | Meta-skill 빌더 (커스텀 스킬 스캐폴드) |
...
autopus-adk/
├── cmd/auto/           # 진입점
├── internal/cli/       # Cobra 명령어 28개 (서브커맨드 포함 55개+)
├── pkg/
...
│   ├── pipeline/       # 파이프라인 상태 지속성 + 체크포인트 재개
│   └── ...             # template, detect, config, version
├── templates/          # 플랫폼별 템플릿
├── content/            # 임베디드 콘텐츠 (16개 에이전트, 37개 스킬)
└── configs/            # 기본 설정
```
